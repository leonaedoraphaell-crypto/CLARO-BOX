...
export const ClaroBoxBrand = ({ className }: { className?: string }) => (
  <div className={`w-full h-full flex items-center justify-center p-2 ${className}`}>
    <div className="relative w-full aspect-square bg-slate-950 rounded-[22%] shadow-2xl border border-white/10 overflow-hidden flex flex-col items-center justify-center">
      {/* Glossy Overlay */}
      <div className="absolute inset-0 bg-gradient-to-br from-white/10 via-transparent to-black/40 pointer-events-none z-20"></div>
      
      {/* Claro Logo & Text */}
      <div className="flex flex-col items-center gap-1 mb-2 relative z-10">
        <div className="flex items-center gap-1.5">
          <div className="w-4 h-4 bg-[#ee2e24] rounded-full flex items-center justify-center shadow-sm border border-white/10">
             <div className="w-2 h-2 bg-white rounded-full"></div>
          </div>
          <span className="text-[7px] font-black text-white tracking-[1px] uppercase">Claro</span>
        </div>
        <span className="text-[6px] font-bold text-white/70 tracking-[4px] uppercase">Box</span>
      </div>

      {/* Front Panel */}
      <div className="w-[80%] h-8 bg-slate-900/80 rounded-lg border border-white/5 shadow-inner flex items-center justify-between px-3 relative z-10">
        {/* Power Button (Left) */}
        <div className="w-3 h-3 rounded-full bg-slate-800 border border-white/10 flex items-center justify-center shadow-sm">
        ...
        <motion.div 
          ...
          className="w-2 h-2 rounded-full bg-[#22c55e] border border-white/10"
        />
      </div>
      
      {/* Streaming Label */}
      <div className="absolute bottom-2 text-[4px] font-black text-blue-500/50 uppercase tracking-[2px]">Streaming Box</div>
    </div>
  </div>
);
