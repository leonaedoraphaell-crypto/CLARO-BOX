...
const AppContent = () => {
  ...
  const [userProfile, setUserProfile] = useState<UserProfile>({ 
    ...
    cpf: '', 
    profileImage: '',
    menuButtonImage: '/api/attachments/08390979-436f-4796-9f88-46637e6f8099'
  });
  ...
  const handleNotificationTouchStart = (id: number) => {
    ...
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-white z-[100]">
      <div className="relative w-32 h-32 mb-8">
        ...
        <motion.div 
          ...
          className="absolute bottom-4 right-8 w-4 h-4 bg-green-500 rounded-full border-2 border-white shadow-[0_0_15px_#22c55e]"
        />
      </div>
    ...
    <div className="flex flex-col min-h-screen relative w-full overflow-x-hidden bg-slate-50 font-sans">
      ...
      <main className="flex-1 px-3 space-y-4 mt-4 pb-48 relative z-10 overflow-visible">
          <div className="space-y-3 overflow-visible">
              <motion.div 
                  ...
                  className={`flex items-center justify-center px-6 py-4 rounded-full shadow-xl border cursor-pointer transition-all duration-500 ${
                      !collapsedCategories[activeCategory] 
                      ? 'bg-gradient-to-r from-blue-600 to-indigo-700 text-white border-blue-400 scale-[1.02]' 
                      : 'bg-white text-slate-800 border-slate-100'
                  }`}
              >
                  <span className="text-base font-black uppercase tracking-[2px]">{activeCategory}</span>
              </motion.div>
      ...
      <AnimatePresence>
        {activeModal === 'settings' && (
            <Modal title="Configurações" onClose={() => setActiveModal(null)}>
                <div className="space-y-8 py-4">
                    <div className="flex flex-col items-center">
                        <div onClick={() => setCameraTarget({ category: activeCategory, item: 'profile' })} className="relative w-28 h-28 rounded-[22%] bg-slate-50 border-2 border-slate-100 overflow-hidden cursor-pointer shadow-inner flex items-center justify-center group">
                    ...
                    <div className="space-y-3 pt-4">
                        <motion.button 
                            ...
                            className="w-full py-5 bg-blue-600 text-white rounded-3xl font-black uppercase tracking-[3px] text-[11px] active:scale-95 transition-all shadow-xl shadow-blue-600/30"
                        >
                            Salvar Perfil
                        </motion.button>
                        <div className="grid grid-cols-2 gap-3">
                            <motion.button 
                                ...
                                className="py-5 bg-red-50 text-red-500 border border-red-100 rounded-3xl font-black uppercase tracking-[2px] text-[9px] active:scale-95 transition-all"
                            >
                                Limpar Perfil
                            </motion.button>
                            <motion.button 
                                ...
                                className="py-5 bg-red-600 text-white rounded-3xl font-black uppercase tracking-[2px] text-[9px] active:scale-95 transition-all shadow-lg shadow-red-600/20"
                            >
                                Limpar Tudo
                            </motion.button>
                        </div>
                    ...
                    {notifications
                      ...
                      .map(n => (
                        <motion.div 
                            key={n.id} 
                            whileTap={{ scale: 0.98 }}
                            onMouseDown={() => handleNotificationTouchStart(n.id)}
                            onMouseUp={handleNotificationTouchEnd}
                            onMouseLeave={handleNotificationTouchEnd}
                            onTouchStart={() => handleNotificationTouchStart(n.id)}
                            onTouchEnd={handleNotificationTouchEnd}
                            onClick={() => handleNotificationClick(n)}
                            className={`p-5 rounded-[2rem] border flex items-center gap-5 shadow-sm transition-all cursor-pointer relative overflow-hidden ${n.read ? 'bg-white opacity-60 border-slate-50' : 'bg-slate-50 border-slate-100'}`}
                        >
                            {notificationToDelete === n.id && (
                                <motion.div 
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    className="absolute inset-0 bg-red-600/95 backdrop-blur-md z-10 flex items-center justify-center gap-4 px-6"
                                ...
                                >
                                    ...
                                    <div className="flex gap-2">
                                        <button 
                                            ...
                                            className="px-4 py-2 bg-white text-red-600 rounded-xl text-[9px] font-black uppercase tracking-widest"
                                        >
                                            Sim
                                        </button>
                                        <button 
                                            onClick={() => setNotificationToDelete(null)}
                                            className="px-4 py-2 bg-white/20 text-white rounded-xl text-[9px] font-black uppercase tracking-widest"
                                        >
                                            Não
                                        </button>
                                    </div>
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-[11px] font-black text-slate-500 leading-tight truncate">{n.details}</p>
                            </div>
                        </motion.div>
        ...
        {selectedNotification && (
            <Modal title="Detalhes da Notificação" onClose={() => setSelectedNotification(null)}>
                <div className="py-6 space-y-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${selectedNotification.type === 'Erro' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}>
                                <IconBell className="w-5 h-5" />
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">{selectedNotification.type}</span>
                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{selectedNotification.time}</span>
                            </div>
                        </div>
                    </div>
                    
                    <div className="bg-slate-50 rounded-[2rem] p-8 border border-slate-100">
                        <p className="text-[13px] font-black text-slate-600 leading-relaxed uppercase tracking-wide">
                            {selectedNotification.details}
                        </p>
                    </div>

                    {selectedNotification.type === 'Erro' && isAdmin && (
                        <div className="space-y-4 pt-4">
                            <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1">Correção Assistida por IA</label>
                            <textarea 
                                placeholder="DESCREVA A CORREÇÃO PARA A IA EXECUTAR..." 
                                className="w-full h-32 py-4 px-6 rounded-2xl bg-slate-50 border border-slate-100 outline-none font-black text-[10px] text-slate-800 placeholder-slate-200 focus:bg-white transition-all shadow-inner resize-none"
                            />
                            <motion.button 
                                whileTap={{ scale: 0.98 }}
                                onClick={() => {
                                ...
                                className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-indigo-600/20"
                            >
                                Corrigir com IA
                            </motion.button>
                        </div>
                    ...
                    <motion.button 
                        ...
                        className="w-full py-4 bg-slate-100 text-slate-600 rounded-2xl font-black uppercase tracking-widest text-[10px] border border-slate-200"
                    >
                        Fechar
                    </motion.button>
                </div>
        ...
        {showAdminLogin && (
            <Modal title="Login Administrador" onClose={() => { setShowAdminLogin(false); setLoginError(false); }}>
                <div className="space-y-6 py-4">
                    <AnimatePresence>
                        {loginError && (
                            <motion.div 
                            ...
                            >
                                ...
                                <span className="text-[9px] font-black text-red-600 uppercase tracking-widest">Usuário ou senha incorretos</span>
                            </motion.div>
                        )}
                    </AnimatePresence>
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <input 
                                ...
                                onChange={e => { setAdminLoginData({...adminLoginData, user: e.target.value}); setLoginError(false); }}
                                onKeyDown={e => e.key === 'Enter' && handleAdminLogin()}
                                className={`w-full py-4 px-6 rounded-2xl bg-slate-50 border outline-none font-black text-[10px] text-slate-800 focus:bg-white transition-all ${loginError ? 'border-red-200' : 'border-slate-100'}`}
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <input 
                                    ...
                                    onChange={e => { setAdminLoginData({...adminLoginData, pass: e.target.value}); setLoginError(false); }}
                                    onKeyDown={e => e.key === 'Enter' && handleAdminLogin()}
                                    className={`w-full py-4 pl-6 pr-14 rounded-2xl bg-slate-50 border outline-none font-black text-[10px] text-slate-800 focus:bg-white transition-all ${loginError ? 'border-red-200' : 'border-slate-100'}`}
                                />
                                <button 
                                    type="button"
                    ...
                    <motion.button 
                        ...
                        className={`w-full py-5 rounded-3xl font-black uppercase tracking-[3px] text-[11px] shadow-xl transition-all ${loginError ? 'bg-red-600 shadow-red-600/20' : 'bg-indigo-600 shadow-indigo-600/20'} text-white`}
                    >
                        Entrar
                    </motion.button>
                </div>
